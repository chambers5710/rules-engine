...
function board(opts: { venusaur: boolean; basic: "bulb" | "ivy" }): GameState {
...
{
  expect(
    listsBreeder(board({ venusaur: true, basic: "bulb" })),
    "Breeder lists with Venusaur in hand and Bulbasaur in play"
  )
  expect(
    !listsBreeder(board({ venusaur: true, basic: "ivy" })),
    "Breeder is omitted when the Stage 2 would land on Ivysaur"
  )
  expect(
    !listsBreeder(board({ venusaur: false, basic: "bulb" })),
    "Breeder is omitted with no Stage 2 in hand"
  )
}

{
  let gamestate = board({ venusaur: true, basic: "bulb" })
