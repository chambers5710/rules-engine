...
- **attackBanned / abilityBanned / attackFlipGated** — `attack_use` (`ban` hides the name — Amnesia is end of turn, Leek Slap is `leave_play`, Slashing Strike is `until.next` plus bench/evolve; `flip` is a machine coin after Confused). `ability_use` `ban` hides that Power (Curse / Step In until owner’s turn ends). `ban: "all"` is every Power on that seat (Stare); `mayUsePokemonPower` is also false
